...
export default function Skill({ symbol, name, desc }) {
  return (
    <div className='group/item cursor-auto border-2 w-[80px] h-[80px] relative flex rounded-md justify-center items-center'>
      ...
      <span className='tooltipText pt-2 pb-3 group-hover/item:opacity-100 group-hover/item:visible transition-all duration-300 top-[80%] last:top-[10px] sm:w-[225px] w-[200px] bg-gray-900 overflow-hidden'>
        <h1 className='text-left font-semibold'>{ name }</h1>
        <p className='text-left ml-2 font-light sm:text-sm text-[12px]'>
          {desc}
        </p>
      </span>
    </div>
